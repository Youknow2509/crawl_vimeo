...
func (y *YoutubeService) UploadVideoBase(ctx context.Context, title, description, privacyStatus, videoPath string) (*youtube.Video, error) {
	// Mở file video
	file, err := os.Open(videoPath)
	if err != nil {
		return nil, fmt.Errorf("không thể mở file video: %w", err)
	}
	defer file.Close()
	// Tạo đối tượng metadata cho video
	video := &youtube.Video{
		Snippet: &youtube.VideoSnippet{
			Title:       title,
			Description: description,
			CategoryId:  "22", // 22 = People & Blogs, xem: https://developers.google.com/youtube/v3/docs/videoCategories/list
		},
		Status: &youtube.VideoStatus{PrivacyStatus: privacyStatus},
	}
	// Upload
	call := global.YTB_SERVICE.Videos.Insert([]string{"snippet", "status"}, video)
	call.Media(file)

	// Quá trình upload có thể mất thời gian
	fmt.Println("Đang upload video, vui lòng đợi...")

	response, err := call.Do()
	if err != nil {
		return nil, fmt.Errorf("không thể upload video: %w", err)
	}
